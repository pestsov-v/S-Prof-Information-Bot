...
# **Оглавление**
  ...
  - [Как я могу развернуть проект у себя](#как-я-могу-развернуть-проект-у-себя)
    - [Получение токена](#получение-токена)
      - [Находим Bot Father в поисковике Telegram](#первый-шаг-находим-bot-father-в-поисковике-telegram)
      - [Запускаем чат с Bot Father](#второй-шаг-запускаем-чат-с-bot-father)
      - [Выбираем функцию "создать нового бота" (/newbot)](#третий-шаг-выбираем-функцию-создать-нового-бота-newbot)
      - [Вводим имя бота](#вводим-имя-бота)
      - [Вводим username бота](#пятый-шаг-вводим-username-бота)
      - [Получаем у Bot Father уникальный токен](#шестой-шаг-получаем-у-bot-father-уникальный-токен)
    - [Получение URL адреса](#-url-адреса)
      - [Находим официальный сайт MongoDB в Google](#первый-шаг-находим-официальный-сайт-mongodb-в-google)
      - [Регистрируем в аккаунт](#второй-шаг-регистрируем-в-аккаунт)
      - [Регистрация через Google аккаунт](#третий-шаг-регистрация-через-google-аккаунт)
      - [Заходим через существующий аккаунт Google](#четвертый-шаг-заходим-через-существующий-аккаунт-google)
      - [Принимаем политику конфиденциальности и условия обслуживания](#пятый-шаг-принимаем-политику-конфиденциальности-и-условия-обслуживания)
      - [Выбираем бесплатный вариант виртуального облака](#шестой-шаг-выбираем-бесплатный-вариант-виртуального-облака)
      - [Создаём виртуальное облако ](#седьмой-шаг-создаём-виртуальное-облако)
      - [Ожидаем пока сформируется база данных](#восьмой-шаг-ожидаем-пока-сформируется-база-данных)
      - [Доступ к базе данных](#девятый-шаг-доступ-к-базе-данных)
      - [Создаём нового пользователя](#десятый-шаг-создаём-нового-пользователя)
      - [Вводим логин и пароль пользователя](#одиннадцатый-шаг-вводим-логин-и-пароль-пользователя)
      - [Доступ адресов](#двенадцатый-шаг-доступ-адресов)
      - [Добавляем новый IP-adress](#тринадцатый-шаг-добавляем-новый-ip-adress)
      - [Выбираем вариант IP-adress](#четырнадцатый-шаг-выбираем-вариант-ip-adress)
      - [Статус IP-adress](#пятнадцатый-шаг-статус-ip-adress)
      - [Кнопка "Connect" ](#шестнадцатый-шаг-кнопка-connect)
      - [Подключение базы данных к приложению](#семнадцатый-шаг-подключение-базы-данных-к-приложению)
      - [URL-ссылка](#восемнадцатый-шаг-url-ссылка)
    - [Формируем config.js с уникальными данными](#формируем-configjs-с-уникальными-данными)
      - [Создаём файл config.js](#cоздаём-файл-configjs)
      - [Подключаем файл config.js](#подключаем-файл-configjs)
      - [Проверки](#проверки)
    - [Загрузка данных в MongoDB](#загрузка-данных-в-mongodb)
  - [Функции](#функции)
    - [Найти проект](#найти-проект)
      - [Первый шаг. Кнопка "Проекти"](#первый-шаг-кнопка-проекты)
      - [Второй шаг. Выбираем желаемый тип объекта](#второй-шаг-выбираем-желаемый-тип-объекта)
      - [Третий шаг. Выводим конкретный проект исходя из ID](#третий-шаг-выводим-конкретный-проект-исходя-из-id)
      - [Четвертый шаг. Отправляем краткую информацию](#четвертый-шаг-отправляем-краткую-информацию)
...
Особенностью любых ботов созданных на платформе Telegram есть, в первую очередь, получения уникального токена, по которому сервера Telegram идентифицируют единую точку входа, запросы или же обновления бота. Это означает, что нельзя развернуть одного и того же бота с одинаковым токеном, а следовательно необходимо получить индивидуальный токен.<br/><br/>
...
#### Первый шаг. Находим Bot Father в поисковике Telegram
...
#### Пятый шаг. Вводим username бота
...
#### Шестой шаг. Получаем у Bot Father уникальный токен
...
#### Второй шаг. Регистрируем в аккаунт

[![2.png](https://i.postimg.cc/DzP953bJ/2.png)](https://postimg.cc/p9ycX7CR)

#### Третий шаг. Регистрация через Google аккаунт
...
#### Четвертый шаг. Заходим через существующий аккаунт Google
...
#### Шестой шаг. Выбираем бесплатный вариант виртуального облака
...
#### Седьмой шаг. Создаём виртуальное облако 
Проверяем и выбираем следующие пункты:
...
#### Восьмой шаг. Ожидаем пока сформируется база данных
...
#### Девятый шаг. Доступ к базе данных

После того, как сформируется база данных, переходим в графу "Доступ к базе данных".
...
#### Пятнадцатый шаг. Статус IP-adress
Проверяем статус IP-adress и переходим в базу данных.
*Статус должен быть изменен с "pending" на "active".*

[![16.png](https://i.postimg.cc/Fsn3dYW1/16.png)](https://postimg.cc/rKxK7wLL)

#### Шестнадцатый шаг. Кнопка "Connect" 
Нажимаем на кнопку "Connect" (подключиться).

[![17.png](https://i.postimg.cc/6p2RdBgc/17.png)](https://postimg.cc/RWmNmxMJ)

#### Семнадцатый шаг. Подключение базы данных к приложению
Выбираем пункт "Connect to your application" (подключиться к Вашему приложению).

[![18.png](https://i.postimg.cc/Pq1wR2t8/18.png)](https://postimg.cc/675q7V49)

#### Восемнадцатый шаг. URL-ссылка
Коппируем URL-ссылку подключения к базе данных MongoDB. Она нам понадобиться позже. <br/>
...
Подключаем сonfig.js в наш главный файл index.js
```
const config = require('./config')
```
#### Проверки
Проверяем первый аргумент класса TelegramBot.
```
const bot = new TelegramBot(config.TOKEN, {
    polling: true
})
```
Проверяем первый аргумент метода connect библиотеки mongoose.
```
...
#### Четвертый шаг. Отправляем краткую информацию

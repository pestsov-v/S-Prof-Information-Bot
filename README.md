...
Практика показала, что организовывать загрузку данных через чат с Telegram ботом является не удобным решением, а формировать обработку загружаемых данных через `CRUD` это создание двух отдельных приложений, когда задача стоит - монолит.
...
4. Иметь доступ к статистике проектов. Статистика проектов - это онлайн Excel таблица, которая заполняется разными сотрудниками паралельно, регистрируя уровень продаж тех или иных продуктов компании, а также другие виды статистики по товарам, такие как поставки по регионам и так далее.

## Запуск бота

Вводим название "S-Prof.Інформаційний бот" в поисковик `Telegram`. Поскольку ранее бот был запущен, поэтому в последующие запуски, он может только перезпаускатся. Вам `Telegram` предложит начать. Нажимаем кнопку `Start`. <br/>
...
Если необходима короткая информация по объекту, в том числе и геоположение необходимо использовать путь через клавишу "Проекти". <br/>
...
При нажатии на клавишу "Проекти" срабатывает следующий блок кода: <br/>
...
*Код других клавиш главного меню ([смотреть перечень клавиш главного меню](#главное-меню)) аналогичен клавише "Скачать проект"*  <br/>
...
Данные обратного вызова используются дальше - в прослушке событий `calback_query` екземпляра класса `TelegramBot`:
```node
...
Клавиша срабатывает на ранее описаный `switch case`, по аналогии с клавишами ["Проекти"](#проекти) или ["Товари"](#товари), но результат отрабатывания функции - вызов функций, которые показывают актуальные товары выбранных пользователем. Блок кода описан ниже:
```node
bot.on('message', msg => {
    const chat_id = function__get_chat_id.get_chat_id(msg)
    switch(msg.text) {
...
Команда отвечающая за запуск главного меню - `/start`, код которой подробно описан в разделе [Запуск бота](#запуск-бота). Код блока выглядит так: 
```node
bot.onText(/\/start/, msg => {
    if (msg.chat.username !== 'AveCardinal') {
        bot.sendSticker(chat_id, stickers.hot_cherry__cry)
...
Логика кода аналогична других клавишах главного меню, таким как [Проекти](#проекти) или [Товари](#товари). Сам код клавиши "Реєстр об'єктів" представляет из себя следующее:
```node
bot.on('message', msg => {
...
Логика кода аналогична других клавишах главного меню, таким как [Проекти](#проекти), [Товари](#товари) или [Реєстр об'єктів](#реестр-проектов). Сам код клавиши "Статистика по об'єктам" представляет из себя следующее:
```node
bot.on('message', msg => {
...
README.md предполагает лишь разбор всех основных функций кода, чтобы сторонний разработчик мог понять тот или другой блок кода. Предполагается, что сторонний разработчик понимает, что такое module.exports и require, следовательно не нуждается в комментировании подключения различных модулей. За дополнительными вопросами можно обращаться ко мне на почту `vladislav.pestsov@gmail.com`.
